  ...
  <div class="h-screen overflow-hidden flex flex-col">
    <nav data-tauri-drag-region class="flex-shrink-0 flex items-center justify-between bg-orange-900">
      <div class="flex items-center">
        <i @click="router.back()" class="bi-chevron-left cursor-pointer hover:bg-orange-800 text-sm px-3.5 py-2.5"></i>
        <i @click="router.forward()" class="bi-chevron-right cursor-pointer hover:bg-orange-800 text-sm px-3.5 py-2.5"></i>
      </div>
      <span class="text-sm absolute left-1/2 -translate-x-1/2">Kafka Panel - {{ title }}</span>
      <div class="flex items-center">
        <i @click="appWindow.minimize()"
          class="bi-dash-lg cursor-pointer hover:bg-orange-800 text-xs px-4 py-3" alt="minimize"></i>
        <i @click="appWindow.toggleMaximize()"
          class="bi-square cursor-pointer hover:bg-orange-800 text-xs px-4 py-3" alt="maximize"></i>
        <i @click="appWindow.close()"
          class="bi-x-lg cursor-pointer hover:bg-orange-800 text-xs px-4 py-3" alt="close"></i>
      </div>
    </nav>
